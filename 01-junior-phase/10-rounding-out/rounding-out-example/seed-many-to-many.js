...
const seed = async () => {
  try {
    ...
    const newTowns = await Promise.all(towns.map(town => Town.create(town)));
    console.log(`${newTowns.length} towns have been created! Let's add some monsters!`);
    const newMonsters = await Promise.all(monsters.map(monster => Monster.create(monster)));
    console.log(`${newMonsters.length} monsters have been created! Place them in their towns!`);
    await Promise.all(newTowns.map(town => town.addMonster(newMonsters[Math.floor(Math.random() * newMonsters.length)])));
    await Promise.all(newMonsters.map(monster => monster.addTown(newTowns[Math.floor(Math.random() * newTowns.length)])));
